...
export default function Navbar({ config }) {
  ...
  function NavItem({ link }) {
    const isAnchorOnly = link.href === "/" && link.anchor !== "#hero";
    if (isHome && isAnchorOnly) {
      return (
        <a
          href={link.anchor}
  ...
  return (
    <>
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            className="fixed inset-0 z-40 bg-gradient-to-b from-flamingo-dark to-black flex flex-col items-center justify-center gap-6"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            ...
            {navLinks.map((link, i) => (
              ...
              >
                <Link
                  to={
                    link.href === "/" && link.anchor !== "#hero"
                      ? `/${link.anchor}`
                      : link.href
                  }
                  onClick={() => setMenuOpen(false)}
                  className="font-display text-4xl text-white hover:text-flamingo transition-colors"
                >
                  {link.label}
                </Link>
              </motion.div>
            ...
            <div className="flex gap-5 mt-8 pt-6 border-t border-white/20">
              {config.instagram && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="text-white/60 hover:text-flamingo transition-colors"
                >
